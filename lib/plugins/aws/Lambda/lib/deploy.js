...
functions.parse_commands = function(input, phase) {
    var commands = [];
    if (input.stack.deploy_lambda.phases) {
        if (input.stack.deploy_lambda.phases[phase] && input.stack.deploy_lambda.phases[phase].commands &&
            !_.isEmpty(input.stack.deploy_lambda.phases[phase].commands)) {
            input.stack.deploy_lambda.phases[phase].commands = _.map(input.stack.deploy_lambda.phases[phase].commands, function(str) {
            ...
            commands = commands.concat(input.stack.deploy_lambda.phases[phase].commands);
        }
    }
    return commands;
};
...
functions.deploy_phase = function(input, done) {
    ...
    if (input.stack.deploy_lambda.phases.deploy.functions && _.isArray(input.stack.deploy_lambda.phases.deploy.functions) &&
        !_.isEmpty(input.stack.deploy_lambda.phases.deploy.functions)) {
        async.eachSeries(input.stack.deploy_lambda.phases.deploy.functions, function (function_obj, next) {
