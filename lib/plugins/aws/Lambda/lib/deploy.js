...
functions.process_commands = function(input, phase, done) {
    if (_.includes(input.params._, 'lambda-unpublish') ||
    ...
    fs.readFile(input.function_obj.zip_file, function (err, data) {
        ...
        lambda.updateFunctionCode(params, function(err, data) {
            if (err) {
                ...
                console.log('deployed function: ' + input.function_obj.function);
                console.log('version = ' + data.Version + ', alias=$LATEST');
                done(null, data);
            }
        });
    });
            ...
            if (err) {
                ...
                lambda.publishVersion(params, function (err, data) {
                    if (err) {
                        ...
                        console.log('published function: ' + input.function_obj.function);
...
functions.deploy_function_version = function(input, done) {
    ...
    lambda.updateAlias(params, function(err, data) {
        if (err) {
            ...
            console.log('deployed function version: ' + input.function_obj.function);
            console.log('version = ' + input.params.version + ', alias='+alias);
            done(null, data);
        }
    });
};

functions.unpublish_function = function(input, done) {
    if (_.isNil(input.function_obj.aliases) || _.isEmpty(input.function_obj.aliases)) {
        return done('Missing \'function.aliases\'.', null);
    }
    var params = {
        FunctionName: input.function_obj.function,
        Qualifier: input.params.version+''
    };
    lambda.deleteFunction(params, function(err, data) {
        if (err) {
            return done(err, null);
        } else {
            console.log('unpublished function version: ' + input.function_obj.function);
            console.log('version = ' + input.params.version + ' has been removed.');
            done(null, data);
        }
    });
};
    ...
    if (input.stack.lambda.functions && _.isArray(input.stack.lambda.functions) &&
        ...
        async.eachSeries(input.stack.lambda.functions, function (function_obj, next) {
            if (!input.params.function || _.isEqual(input.params.function, function_obj.function)) {
                ...
                } else if (_.includes(input.params._, 'lambda-unpublish')) {
                    functions.unpublish_function({function_obj: function_obj, params: input.params}, next);
...
functions.deploy = function(input, done) {
    ...
    if (input.stack.lambda.deploy) {
        if (_.includes(input.params._, 'lambda-deploy')) {
            ...
            console.log('Unpublishing: ' + input.stack.stack.name);
        }
        async.waterfall([
